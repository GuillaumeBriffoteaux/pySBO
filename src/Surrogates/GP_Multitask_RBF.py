...
#---------------------------Defining Gaussian Process Multitask RBF---------------------------#
class MultitaskGPModel(gpytorch.models.ExactGP):
    def __init__(self, x_train, y_train, likelihood):
        super(MultitaskGPModel, self).__init__(x_train, y_train, likelihood)
        self.mean_module = gpytorch.means.MultitaskMean(gpytorch.means.ConstantMean(), num_tasks=y_train.shape[1])
        self.covar_module = gpytorch.kernels.MultitaskKernel(gpytorch.kernels.RBFKernel(), num_tasks=y_train.shape[1], rank=1)
...
#------------------------------------------------#
#-------------class GP_Multitask_RBF-------------#
#------------------------------------------------#
class GP_Multitask_RBF(Surrogate):
    """Class for Gaussian Process multitask model with Radial Basis kernel function (multi dimensional targets only).
    ...
    def __init__(self, f_sim_archive, pb, n_train_samples, f_train_log, f_trained_model):
        ...
        self.__model = MultitaskGPModel(x_train, y_train, self.__likelihood)
    ...
    def _get_likelihood(self):
        print("[GP_Multitask_RBF.py] Impossible to modify the likelihood")
        return None
    ...
    def _del_likelihood(self):
        print("[GP_Multitask_RBF.py] Impossible to delete the likelihood")

    #-------------_get_model-------------#
    def _get_model(self):
        print("[GP_Multitask_RBF.py] Impossible to modify the model")
        return None
    ...
    #-------------_del_model-------------#
    def _del_model(self):
        print("[GP_Multitask_RBF.py] Impossible to delete the model")
