...
#-------------------------------------------------#
#-------------class Best_Predicted_EC-------------#
#-------------------------------------------------#
class Best_Predicted_EC(Informed_EC):
    """Class for best predicted EC.

    Candidates with lower predicted cost (i.e. objective value
    predicted by the surrogate) are more promising.
    """
    ...
    #-------------__str__-------------#
    def __str__(self):
        return "Best Predicted Adaptive Evolution Control\n  surrogate: {"+self.surr.__str__()+"}"
