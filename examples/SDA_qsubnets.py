...
from Surrogates.q_BNN_MCD import q_BNN_MCD
...
def main():
    ...
    POP_SIZE=50
    if q%nprocs!=0:
        print("[SDA_qsubnets.py] error q should be a multiple of the number of processors")
        assert False
    nb_sim_per_proc=q//nprocs
    if nb_sim_per_proc>POP_SIZE:
        print("[SDA_qsubnets.py] number of candidate per proc cannot be greater than the population size")
        assert False
    N_GEN=100
    N_CHLD=POP_SIZE
    ...
    if rank==0:
        ...
        # Creating surrogate
        surr = q_BNN_MCD(F_SIM_ARCHIVE, p, float('inf'), F_TRAIN_LOG, F_TRAINED_MODEL, "master", 1, 1024, F_INIT_WEIGHTS)
        ...
        for curr_cycle in range(N_CYCLES):
            ...
            if TIME_BUDGET>0:
                t_now = time.time()
                if TIME_BUDGET-(t_now-t_start)-nb_sim_per_proc*(curr_cycle+1)*SIM_TIME<SIM_TIME:
                    break
    ...
    #---------------------------------#
        ...
        # Creating surrogate
        surr = q_BNN_MCD(F_SIM_ARCHIVE, p, float('inf'), F_TRAIN_LOG, F_TRAINED_MODEL, "worker", 1, 1024)
