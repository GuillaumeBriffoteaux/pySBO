"""Script running a synchronous parallel Surrogate-Driven Algorithm similar to q-EGO with surrogate believer for single-objective optimization.
...
To run sequentially: ``python3.9 ./SDA_qEGO_SB.py``

To run in parallel (in 4 computational units): ``mpiexec -n 4 python3.9 SDA_qEGO_SB.py``

To run in parallel (in 4 computational units) specifying the units in `./hosts.txt`: ``mpiexec --machinefile ./host.txt -n 4 python3.9 SDA_qEGO_SB.py``
"""
...
def main():
    ...
    if rank==0:
        ...
        # else:
        #     print("[SDA_qEGO_SB.py] error invalid ec name")
        #     assert False
        ...
        for curr_cycle in range(N_CYCLES):
            ...
            for curr_sub_cycle in range(q):
                ...
                pop.obj_vals[0:1] = surr.perform_prediction(pop.dvec[0:1])[0]
                pop.obj_vals[0:1] = surr.denormalize_predictions(pop.obj_vals[0:1])
